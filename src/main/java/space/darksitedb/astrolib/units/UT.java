...
public class UT extends Date {
    ...
    public JulianDate toJulianDayNumber() {
        ...
        double dayFraction = (getHour().getValue() + getMinute().getValue() / 60.0 + getSecond().getValue() / 3600.0) / 24.0;
        jd += dayFraction;
    ...
    public LCT toLCT(int timeZoneOffset) {
        ...
        java.time.LocalDateTime utcDateTime = java.time.LocalDateTime.of(getYear().getValue(), getMonth().getValue(), getDay().getValue(), (int) getHour().getValue(), (int) getMinute().getValue(), (int) getSecond().getValue());
